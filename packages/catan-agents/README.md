...
Two-player strength (seat-swapped, 200–800-game matches): `planner` and `mcts` are tied at the top — planner vs mcts reads 49.5% pooled, and both beat `lookahead` (planner ~55%, mcts 56%) — all well over `greedy` (planner 85%, lookahead 87% against it), with greedy beating random 99%. With seats rotated, one lookahead wins 65% against two greedies at three players (chance 33%), and one mcts wins 62% against two lookaheads (36% against three at four players, chance 25% — measured before the June 12 trade/value upgrade, which both sides share).
