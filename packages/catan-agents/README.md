...
Two-player strength (seat-swapped, 200–300-game matches): `planner` and the search agents are jointly top — planner edges lookahead 52% and ties mcts 50%, while mcts beats lookahead 56% — all well over `greedy` (planner 85%, lookahead 87% against it), with greedy beating random 99%. With seats rotated, one lookahead wins 65% against two greedies at three players (chance 33%), and one mcts wins 62% against two lookaheads (36% against three at four players, chance 25% — measured before the June 12 trade/value upgrade, which both sides share).
