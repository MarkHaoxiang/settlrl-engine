...
def test_bounds_bracket_the_truth(n_players: int) -> None:
    env = BatchedCatanEnv(batch_size=4, seed=0, n_players=n_players, track_beliefs=True)
...
def test_dev_pool_conservation(n_players: int) -> None:
    env = BatchedCatanEnv(batch_size=4, seed=1, n_players=n_players, track_beliefs=True)
    _rollout(env, 150)
    state = env.board[1]
...
def test_two_player_beliefs_are_exact() -> None:
    ...
    env = BatchedCatanEnv(batch_size=4, seed=2, n_players=2, track_beliefs=True)
