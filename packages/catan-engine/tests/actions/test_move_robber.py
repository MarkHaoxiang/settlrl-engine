...
from catan_engine.action_vec import MoveRobber
...
def _robber_fixture(tile: int = 0) -> Board:
    """MOVE_ROBBER board: player 1 sits on ``tile`` with 1 sheep; robber elsewhere."""
...
class TestMoveRobber(TestCase):
    def test_success(self) -> None:
        T = 0
        board = _robber_fixture(T)
        state, result = MoveRobber()(board, (jnp.array([T]), jnp.array([1])))
...
p1_sheep=0""",
    ...
    def test_invalid_wrong_phase(self) -> None:
        T = 0
        board = to_main(make_board(seed=0))  # MAIN, not MOVE_ROBBER
    ...
    def test_invalid_out_of_range_tile(self) -> None:
        board = _robber_fixture(0)
        state, result = MoveRobber()(board, (jnp.array([999]), jnp.array([1])))
        assert int(result[0]) == ActionResult.INVALID.value
