"""catan-learn skeleton: featurization, protocol adapters, trainability."""
...
def test_features_distinguish_players_and_jit() -> None:
    env = BatchedCatanEnv(batch_size=1, n_players=2, seed=3)
...
def test_stand_ins_satisfy_the_seams_and_play_legally() -> None:
    ...
    env = BatchedCatanEnv(batch_size=2, n_players=2, track_beliefs=True, seed=1)
    mask = env.flat_mask()
    layout = env.board[0]
    view = jax.tree.map(
