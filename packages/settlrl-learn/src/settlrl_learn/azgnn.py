...
from settlrl_engine.board.state import GamePhase, IntScalar
...
from settlrl_learn.nn.graphnet import PRESETS, GraphNetConfig
...
def arena(
    ...
    *,
    ...
    value_fn, prior_fn = gnn_seams(model)
    net = make_net_agent(
        value_fn, prior_fn,
        num_simulations=num_simulations,
        max_num_considered_actions=max_num_considered_actions,
...
def learn(
    *,
    ...
    on_iter: Callable[[int, dict[str, float], BoardGNN], None] | None = None,
) -> BoardGNN:
    """An AlphaZero loop over an :class:`BoardGNN`: each iteration self-plays,
    ...
    model0 = BoardGNN(jax.random.key(seed), cfg)
