"""Profiler for the batched Settlrl environment's random-rollout loop.

Answers "where does the wall-clock go when stepping ``BatchedSettlrlEnv`` under
...
import jax
from settlrl_engine.env.batched import N_FLAT, BatchedSettlrlEnv


def _rollout(env: BatchedSettlrlEnv, key: jax.Array, steps: int) -> jax.Array:
...
def _warmup(env: BatchedSettlrlEnv, key: jax.Array, steps: int = 2) -> jax.Array:
...
def run_cprofile(batch_size: int, steps: int, seed: int, top: int) -> None:
    env = BatchedSettlrlEnv(batch_size=batch_size, seed=seed)
