...
The game engine for Settlrl: it generates boards, knows the rules, and applies player actions to advance a game.
...
extra: `pip install 'settlrl-engine[cuda]'`.
...
- **Game records** — `settlrl_engine.record` saves a complete game as readable JSON (the configuration plus every move played, with dice outcomes) and replays it deterministically: `record_game` plays and records a game, `GameRecord.to_json` / `from_json` serialise it, and `replay` re-steps it move by move, verifying it against the engine as it goes.
...
```bash
uv run --package settlrl-engine pytest
```
