...
- ``BatchedSettlrlEnv`` -- a batch of games stepped in lockstep (the vectorised
...
- ``SettlrlAECEnv`` -- a single game, turn at a time (the PettingZoo surface).
...
``BatchedSettlrlEnv.random_actions`` and the AEC rollout via the action mask, so
...
from settlrl_engine.env.batched import BatchedSettlrlEnv
...
def _batched_rollout(
    ...
    with jax.default_device(jax.devices(device)[0]):
        env = BatchedSettlrlEnv(batch_size=batch_size, n_players=n_players, seed=seed)
...
def _aec_rollout(seed: int, n_players: int, steps: int, device: str) -> None:
    with jax.default_device(jax.devices(device)[0]):
        e = SettlrlAECEnv(n_players=n_players, seed=seed)
