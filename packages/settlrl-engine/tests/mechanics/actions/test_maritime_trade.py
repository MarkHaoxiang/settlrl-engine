...
from settlrl_engine.board import Board, give, make_board, place_settlement, to_main
from settlrl_engine.board.layout import PORT_V
...
from settlrl_engine.mechanics.trade import maritime_step
...
def test_success(trade_board: Board) -> None:
    state, result = maritime_step(trade_board, (jnp.array([0]), jnp.array([1])))
...
def test_success_specific_port_2to1() -> None:
    ...
    state, result = maritime_step(board, (jnp.array([0]), jnp.array([1])))
    assert_expected_inline(
        fmt(
            result,
            sheep=int(state.player_resources[0, 0, 0]),
...
def test_invalid_give_equals_receive(trade_board: Board) -> None:
    ...
    state, result = maritime_step(trade_board, (jnp.array([0]), jnp.array([0])))
    assert int(result[0]) == ActionResult.INVALID.value
    assert np.array_equal(np.asarray(state.player_resources), before)
