"""Text rendering of a Settlrl board, for human-readable expect-test snapshots.
...
The projection
``row = 3a - (a+b+c)``, ``col = c - b`` lays the standard Settlrl hexagon out as a
...
import numpy as np
from settlrl_engine.board.dev_cards import N_DEV_CARD_TYPES, DevCard
...
from settlrl_engine.board.resources import N_PLAYERS, compute_bank_resources
...
class BoardRenderer:
    """Renders a full Settlrl board (layout + state) as text.
