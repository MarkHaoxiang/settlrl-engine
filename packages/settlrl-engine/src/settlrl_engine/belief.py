...
An optional companion to ``BoardState`` (``BatchedSettlrlEnv(track_beliefs=
...
Settlrl: the *type* of a card moved by a robber steal (thief and victim see it)
...
not steppable -- the road back to a playable position is settlrl-agents'
``sample_world``.
"""
...
from settlrl_engine.mechanics.action import ActionParams, ActionType
...
class BeliefView(NamedTuple):
    ...
    posterior sample (settlrl-agents' ``sample_world``) first.
    """
