...
from settlrl_engine.board.layout import EDGE_V, N_EDGES, N_VERTICES, BoardLayout
...
def build_road_step(board: Board, edge: IndexParam) -> tuple[BoardState, ResultCode]:
    ...
    needed = awards.road_build_needed_b(state, result)
    return cast(
...
def build_settlement_step(
    ...
    needed = awards.settlement_break_needed_b(state, vertex, result)
    return cast(
        "tuple[BoardState, ResultCode]",
        awards.resolve_step_b(state, result, needed),
    )
