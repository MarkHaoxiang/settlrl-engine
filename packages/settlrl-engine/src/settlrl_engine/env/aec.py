...
``SettlrlAECEnv`` drives one game via ``BatchedSettlrlEnv(batch_size=1,
auto_reset=False)`` (real terminal states); for vectorised rollouts use
``BatchedSettlrlEnv`` directly. The action space is one flat ``Discrete`` over
...
# ``mechanics/flat.py`` -- shared with ``BatchedSettlrlEnv.random_actions``.


class SettlrlAECEnv(AECEnv):  # type: ignore[misc]  # pettingzoo is untyped (Any base)
    """A single Settlrl game behind the PettingZoo AEC API.

    Args:
        seed: PRNG seed for the board and dice/steal randomness.
        reward: ``"sparse"`` or ``"vp_delta"`` (see ``BatchedSettlrlEnv``).
