...
Settlrl-playing agents for [settlrl-engine](../settlrl-engine).

Agents consume the engine's flat action space (decode a chosen index with `settlrl_engine.env.flat_to_action`) and come in three kinds, split by what a seat consumes and how it runs — none sees anything the player wouldn't:
...
- `mcts` — re-determinizing Single-Observer ISMCTS over a custom tree (Gumbel-MuZero selection), using the engine as its simulator and the value function at the leaves; it samples a fresh world from the seat's belief each simulation and filters legality per simulation, so the search integrates over opponents' hidden cards rather than fixing one guess. `lookahead` is the same search at zero simulations.
...
`settlrl-agents compare <a> <b>` plays two agents head-to-head (100 games by default, seats swapped halfway to cancel the first-mover advantage):

```
$ settlrl-agents compare mcts greedy
...
`settlrl-agents bench <a> <b>` is the experiment harness: each side is a registry name or a JSON spec configuring a family — builder knobs under `"params"`, heuristic weight overrides under `"value"`. Two-player runs swap seats; `--players 3|4` rotates agent `a` through every seat against a table of `b`:

```
$ settlrl-agents bench '{"kind": "mcts", "params": {"num_simulations": 64}}' lookahead --games 200
